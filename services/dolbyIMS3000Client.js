...
class DolbyIMS3000Client {
    ...
    async getSchedule() {
        try {
            ...
            return { schedule: [], referenceDate: null };
        } catch (err) {
    ...
    parseScheduleHTML(html) {
        ...
        const schedules = [];
        let referenceDate = null; // First date found

        $('.createScheduleDiv').each((i, el) => {
            const $dateDiv = $(el);
            const dateStr = $dateDiv.attr('attr-date'); // "02-16-2026"

            // Try to use the first date encountered as reference
            if (!referenceDate && dateStr) {
                const [mm, dd, yyyy] = dateStr.split('-');
                referenceDate = new Date(`${yyyy}-${mm}-${dd}`);
            }
        ...
        return { schedule: schedules, referenceDate };
    }
