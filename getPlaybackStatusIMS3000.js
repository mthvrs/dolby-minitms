/**
 * getPlaybackStatusIMS3000.js
 * Retrieves current playback status from Dolby IMS3000 server
...
/**
 * Extracts the SOAP session ID from the IMS3000 playback page HTML
...
async function extractSoapSessionId(session, theaterConfig) {
    // IMS3000 uses a different URL structure with query parameter
...
/**
 * Gets current playback status from the Dolby IMS3000 server
 */
async function getPlaybackStatus(theaterName) {
    ...
    try {
        ...
        // Step 4: Request playback status (note the different Referer URL for IMS3000)
        const response = await session.request('POST', '/dc/dcp/json/v1/ShowControl', soapBody, {
            ...
            'Referer': `${theaterConfig.url}/web/index.php?page=sys_control/cinelister/playback.php`
        });
...
if (require.main === module) {
    (async () => {
        try {
            const playbackInfo = await getPlaybackStatus('Salle 1');
            
            console.log('\n=== PLAYBACK STATUS (IMS3000) ===');
            ...
            if (playbackInfo.stateInfo) {
                ...
                console.log(`CPL: ${playbackInfo.cplTitle || 'N/A'}`);
            }
